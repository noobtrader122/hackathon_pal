# hackathon_pal/models/sqlalchemy_models.py
"""
...
db = SQLAlchemy()          # imported and initialised in __init__.py
...
class Hackathon(db.Model):
    ...
    challenges = db.relationship('Challenge', backref='hackathon', lazy=True)
...
class TestCase(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("challenge_id", "test_id",
                                          name="uix_challenge_testid"),)
...
class Submission(db.Model, TimestampMixin):
    ...
    id             = db.Column(db.Integer, primary_key=True)
    submission_uid = db.Column(db.String(36),
    ...
    status         = db.Column(db.Enum("pending", "correct", "incorrect",
                                       "timeout", "error", "exceeded_limit",
    ...
    tc_results  = relationship("TestCaseResult",
...
class TestCaseResult(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("submission_id", "test_id",
                                          name="uix_sub_test"),)
    ...
    test_id          = db.Column(db.Integer, nullable=False)
    status           = db.Column(db.Enum("correct", "incorrect",
                                         "timeout", "error",
                                         name="tc_status_enum"),
                                 nullable=False)
